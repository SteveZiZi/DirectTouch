...
public class TaskSetService extends Service {
	 ...
	 * 
	 */
	MyHandler handler = new MyHandler(TaskSetService.this) {
		@Override
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (isenable) {
			switch (curCPC) {
			case 1:
				new CPC1TaskSet().start();
				break;

			case 2:
				new CPC2TaskSet().start();
				break;

			case 3:
				new CPC3TaskSet().start();
				break;

			case 4:
				new CPC4TaskSet().start();
				break;

			case 5:
				new CPC5TaskSet().start();
				break;

			case 6:
				new CPC6TaskSet().start();
				break;
	...
	private void InitIP() {
		...
		for (int i = 0; i < 6; ++i) {
			...
			mlogic[i] = sp.getInt("logicAddr" + (i + 1), 1);
		}
	}

	public class CPC1TaskSet extends Thread {
		public void run() {
			if (maddr[0]) {
	...
	public class CPC2TaskSet extends Thread {
		public void run() {
			if (maddr[1]) {
				ProtocolData pdsend = new ProtocolData(controlByte, usefulData);
	...
	public class CPC3TaskSet extends Thread {
		public void run() {
			if (maddr[2]) {
				ProtocolData pdsend = new ProtocolData(controlByte, usefulData);
	...
	public class CPC4TaskSet extends Thread {
		public void run() {
			if (maddr[3]) {
				ProtocolData pdsend = new ProtocolData(controlByte, usefulData);
	...
	public class CPC5TaskSet extends Thread {
		public void run() {
			if (maddr[4]) {
				ProtocolData pdsend = new ProtocolData(controlByte, usefulData);
	...
	public class CPC6TaskSet extends Thread {
		public void run() {
			if (maddr[5]) {
				ProtocolData pdsend = new ProtocolData(controlByte, usefulData);
	...
	public void task(byte[] mess) {
		...
		Intent intent = new Intent("action.TaskSetService");
